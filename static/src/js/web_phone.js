              ...
              <t t-foreach="state.contacts" t-as="contact">
                <tr t-key="contact.id">
                  ...
                  <td class="o_contact_info">
                    ...
                    <t t-if="contact.phone_normalized">
                      <div>
                        <div class="fa fa-phone o_phone_number_back"/>
                        <span class="o_contact_phone"><t t-esc="contact.phone_normalized"/></span>
                      </div>
                    </t>
                    <t t-if="contact.mobile_normalized">
                      <div>
                        <div class="fa fa-mobile o_phone_number_back"/>
                        <span class="o_contact_phone"><t t-esc="contact.mobile_normalized"/></span>
                      </div>
                    </t>
                  </td>
                  <td class="o_contact_call_button">
                    <div style="display: flex;">
                      <div style="width: 40px; height: 30px;">
                        <t t-if="contact.mobile_normalized">
                          <t t-if="state.isTransfer">
                            <button aria-label="Call" title="Transfer" t-att-contact-phone="contact.mobile_normalized" t-on-click="_onClickMakeTransfer">
                              <i class="fa fa-mobile" t-att-contact-phone="contact.mobile_normalized"/>
                            </button>
                          </t>
                          <t t-else="">
                            <button aria-label="Call" title="Call" t-att-contact-phone="contact.mobile_normalized" t-on-click="_onClickContactCall">
                              <i class="fa fa-mobile" t-att-contact-phone="contact.mobile_normalized"/>
                            </button>
                          </t>
                        </t>
                      ...
                      <t t-if="contact.phone_normalized">
                        <t t-if="state.isTransfer">
                          <button aria-label="Call" title="Transfer" t-att-contact-phone="contact.phone_normalized" t-on-click="_onClickMakeTransfer">
                            <i class="fa fa-phone" t-att-contact-phone="contact.phone_normalized"/>
                          </button>
                        </t>
                        <t t-else="">
                          <button aria-label="Call" title="Call" t-att-contact-phone="contact.phone_normalized" t-on-click="_onClickContactCall">
                            <i class="fa fa-phone" t-att-contact-phone="contact.phone_normalized"/>
                          </button>
                        </t>
                      </t>
...
<!--Incoming buttons-->
    ...
    `
        ...
        async willStart() {
            ...
            this.beepPlayer = document.createElement("audio");
            this.beepPlayer.volume = 1;
            this.beepPlayer.setAttribute("src", "/asterisk_plus/static/src/sounds/beep.mp3");
            ...
            self._userAgent.on("newRTCSession", function ({session}) {
                ...
                if (session.direction === "incoming") {
                    if (self.state.inCall) {
                        // console.log('Terminate inCall')
                        session.terminate();
                        return;
                    }

                    const answerMode = session._request.headers['Answer-Mode'];
                    let autoAnswer = false;
                    if (answerMode) {
                        autoAnswer = answerMode[0].raw === 'Auto';
                    }
                    const phone_number = session._request.from._uri._user;
        ...
        searchContact() {
            ...
            let contacts = this.state.raw_contacts.filter(o => {
                return o.name.toLowerCase().includes(search_query.toLowerCase()) | (o.mobile_normalized ? o.mobile_normalized.includes(search_query.toLowerCase()) : false) | (o.phone_normalized ? o.phone_normalized.includes(search_query.toLowerCase()) : false)
            });
        ...
        async get_contacts() {
            const contacts = await this.env.services.rpc({
                ...
                fields: ['id', 'name', 'email', 'phone_normalized', 'mobile_normalized', 'image_128'],
            });
            this.state.contacts = contacts;
